 ...
 *************************************************/
INT32 get_d07_ruler_id(E_D07_RULER_TYPE type, S_D07_RULER_PARA para,
		UINT32 *poutRulerID) {
 ...
 *************************************************/
INT32 pack_d07_frame_by_data(S_D07_PACK_FRAME *inPara, char *outBuffer,
		INT32 *outLength) {
	INT32 i;
 ...
 *************************************************/
INT32 unpack_d07_frame(void *inpBuffer, INT32 inBufferLength,
	...
	if (E_D07_CTRL_SR_OK == outpFrame->ctrl_s.reply) {
		...
		// 规约ID
		ulRulerID = ((aucDataTmp[0] & 0xFF) | ((aucDataTmp[1] << 8) & 0xFF00)
...
INT32 get_d07_ruler_info(UINT32 rulerID, S_D07_RULER_INFO *outRulerInfo) {
		...
		case 0: // [04]{00}[*][*]
		{
			...
			case 5: // [04][00]{05}[*]
			{
				if (0xFF == ucDi0) // [04][00][05]{FF}
					...
					type = E_D07_RULER_TYPE_METER_RUN_STWD_1
							+ (E_D07_RULER_TYPE) (ucDi0 - 1);
					len = 2;
					format = E_D07_FMT_XX_XX;
					func = trans_d07_data_XX_XX;
			...
			case 0xA: // [04][00]{0A}[*]
			{
				if (1 == ucDi0) // [04][00][0A]{01}
					...
					sprintf(name_1, "负荷记录起始时间");
						...
						{
					type = E_D07_RULER_TYPE_OVERLOAD_INTRV_TIME_1
							+ (E_D07_RULER_TYPE) (ucDi0 - 2);
					len = 2;
					format = E_D07_FMT_NNNN;
					func = trans_d07_data_NNNN;
			...
			case 0xC: // [04][00]{0C}[*]
			{
				if (ucDi0 >= 1 && ucDi0 <= 0xA) // [04][00][0C]{(1~A)}
						{
					type = E_D07_RULER_TYPE_PASSWORD_LV0
							+ (E_D07_RULER_TYPE) (ucDi0 - 1);
					len = 4;
	...
	case 6: //{06}[*][*][*]
	{
		...
		} else  //[06]{(1~6)}[00][*]
		{
			...
			{
			case 0: {
				type = E_D07_RULER_TYPE_EARLY_OVERLOAD_RECORD_1
						+ (E_D07_RULER_TYPE) (ucDi2 - 1);
				len = 1;
				format = E_D07_FMT_NN;
				func = trans_d07_data_NN;
			...
			case 1: //[06][(1~6)][00]{02}
			{
				type = E_D07_RULER_TYPE_GIVEN_OVERLOAD_RECORD_1
						+ (E_D07_RULER_TYPE) (ucDi2 - 1);
				len = 6;
				format = E_D07_FMT_YYMMDDhhmmNN;
				func = trans_d07_data_YYMMDDhhmmNN;
			...
			case 2: //[06][(1~6)][00]{02}
			{
				type = E_D07_RULER_TYPE_RECNT_OVERLOAD_RECORD_1
						+ (E_D07_RULER_TYPE) (ucDi2 - 1);
				len = 1;
				format = E_D07_FMT_NN;
				func = trans_d07_data_NN; // 固定值01
...
INT32 d07_remove_dot(INT32 len, char*inArray, char *outArray) {
	...
	for (i = 0; j < len; i++) {
		if ((inArray[i] < '0') || ((inArray[i] > '9') && (inArray[i] < 'A'))
...
eD07Err trans_d07_data_XX_XXXX_8(E_D07_TRANS_FLG flag, char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(acTmp, "%2.4f%2.4f%2.4f%2.4f%2.4f%2.4f%2.4f%2.4f",
				pDataUser->fXX_XXXXX[0], pDataUser->fXX_XXXXX[1],
				pDataUser->fXX_XXXXX[2], pDataUser->fXX_XXXXX[3],
				pDataUser->fXX_XXXXX[4], pDataUser->fXX_XXXXX[5],
				pDataUser->fXX_XXXXX[6], pDataUser->fXX_XXXXX[7]);
...
eD07Err trans_d07_data_YYMMDDhhmmss_XXX_XXX_YYMMDDhhmmss(E_D07_TRANS_FLG flag,
		char *user, char *frame) {
	S_D07_YYMMDDhhmmss_XX_XXXX_YYMMDDhhmmss *pDataUser =
			(S_D07_YYMMDDhhmmss_XX_XXXX_YYMMDDhhmmss *) user;
	char str[300] = { 0 };
	char strOut[31] = { 0 };
	INT32 i = 0;
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(str,
				"%02d%02d%02d%02d%02d%02d"        //sBgnTime
						...
						"%06.3f",//fXXX_XXX
				//sBgnTime
				pDataUser->sBgnTime.YY, pDataUser->sBgnTime.MM,
				pDataUser->sBgnTime.DD, pDataUser->sBgnTime.hh,
				pDataUser->sBgnTime.mm, pDataUser->sBgnTime.ss,
				//sEndTime
				pDataUser->sEndTime.YY, pDataUser->sEndTime.MM,
				pDataUser->sEndTime.DD, pDataUser->sEndTime.hh,
				pDataUser->sEndTime.mm, pDataUser->sEndTime.ss,
				//fXXX_XXX
				pDataUser->fXXX_XXX);
...
eD07Err trans_d07_data_YYMMDDhhmmss_2_XX_XXXX_YYMMDDhhmm(E_D07_TRANS_FLG flag,
		char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(str,
				...
				pDataUser->sEndTime.mm, pDataUser->sEndTime.ss,
				//sDemand
				pDataUser->sDemand.fXX_XXXX, pDataUser->sDemand.YY,
				pDataUser->sDemand.MM, pDataUser->sDemand.DD,
				pDataUser->sDemand.hh, pDataUser->sDemand.mm);
...
eD07Err trans_d07_data_YYMMDDhhmmss_2(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		sprintf(acTmp, "%02d%02d%02d%02d%02d%02d"
				"%02d%02d%02d%02d%02d%02d", pYYMMDDhhmmss_2->sBegin.YY,
				pYYMMDDhhmmss_2->sBegin.MM, pYYMMDDhhmmss_2->sBegin.DD,
				pYYMMDDhhmmss_2->sBegin.hh, pYYMMDDhhmmss_2->sBegin.mm,
				pYYMMDDhhmmss_2->sBegin.ss, pYYMMDDhhmmss_2->sEnd.YY,
				pYYMMDDhhmmss_2->sEnd.MM, pYYMMDDhhmmss_2->sEnd.DD,
				pYYMMDDhhmmss_2->sEnd.hh, pYYMMDDhhmmss_2->sEnd.mm,
...
eD07Err trans_d07_data_YYMMDDhhmm(E_D07_TRANS_FLG flag, char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		sprintf(acTmp, "%02d%02d%02d%02d%02d", pYYMMDDhhmmNN->YY,
				pYYMMDDhhmmNN->MM, pYYMMDDhhmmNN->DD, pYYMMDDhhmmNN->hh,
				pYYMMDDhhmmNN->mm);
...
eD07Err trans_d07_data_YYMMDDhhmmNN(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		sprintf(acTmp, "%02d%02d%02d%02d%02d%02d", pYYMMDDhhmmNN->YY,
				pYYMMDDhhmmNN->MM, pYYMMDDhhmmNN->DD, pYYMMDDhhmmNN->hh,
				pYYMMDDhhmmNN->mm, pYYMMDDhhmmNN->NN);
#ifdef D07_DEBUG_ON
		if(D07_ON == g_need_pack_07)
		{
			memset((void*)acTmp, 0x00, 12);
...
eD07Err trans_d07_data_YYMMDDhhmmss(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		sprintf(acTmp, "%02d%02d%02d%02d%02d%02d", pYYMMDDhhmmss->YY,
				pYYMMDDhhmmss->MM, pYYMMDDhhmmss->DD, pYYMMDDhhmmss->hh,
				pYYMMDDhhmmss->mm, pYYMMDDhhmmss->ss);
...
eD07Err trans_d07_data_hhmmss(E_D07_TRANS_FLG flag, char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(acTmp, "%02d%02d%02d", phhmmss->hh, phhmmss->mm, phhmmss->ss);
...
eD07Err trans_d07_data_DDhh(E_D07_TRANS_FLG flag, char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		sprintf(acTmp, "%02d%02d", pDDhh->DD, pDDhh->hh);
...
eD07Err trans_d07_data_MMDDhhmm(E_D07_TRANS_FLG flag, char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(acTmp, "%02d%02d%02d%02d", pMMDDhhmm->MM, pMMDDhhmm->DD,
				pMMDDhhmm->hh, pMMDDhhmm->mm);
...
eD07Err trans_d07_data_YYMMDDWW(E_D07_TRANS_FLG flag, char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(acTmp, "%02d%02d%02d%02d", pYYMMDDWW->YY, pYYMMDDWW->MM,
				pYYMMDDWW->DD, pYYMMDDWW->WW);
...
eD07Err trans_d07_data_YYMMDDNN(E_D07_TRANS_FLG flag, char *user, char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		sprintf(acTmp, "%02d%02d%02d%02d", pYYMMDDNN->YY, pYYMMDDNN->MM,
				pYYMMDDNN->DD, pYYMMDDNN->NN);
...
// XX.XXXXYYMMDDhhmm
eD07Err trans_d07_data_XX_XXXX_YYMMDDhhmm(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(str, "%2.4f%d%d%d%d%d", pDataUser->fXX_XXXX, pDataUser->YY,
				pDataUser->MM, pDataUser->DD, pDataUser->hh, pDataUser->mm);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_loss_less_volt(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
...
#define BUF_OUT_SIZE    262
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(strUsr,
				...
				//sFactorA
				pDataUser->sFactorA.fVolt, pDataUser->sFactorA.fElec,
				...
				//sFactorB
				pDataUser->sFactorB.fVolt, pDataUser->sFactorB.fElec,
				...
				//sFactorA
				pDataUser->sFactorC.fVolt, pDataUser->sFactorC.fElec,
				pDataUser->sFactorC.fHavePowerRate,
				pDataUser->sFactorC.fNonePowerRate,
				pDataUser->sFactorC.fPowerRateFactor,
				// sAtime
				pDataUser->sAtime.dAtimeTotal, pDataUser->sAtime.dAtimePhaseA,
				pDataUser->sAtime.dAtimePhaseB, pDataUser->sAtime.dAtimePhaseC); //297
	...
	} else                        // frame to user interface
	{
		...
		pDataUser->sPowerTotal.dForthHavePowerIcr = d07_str_to_double(
				&strUsr[i += 2], 8, 6);
		pDataUser->sPowerTotal.dBackHavePowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerTotal.dCombNone1PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerTotal.dCombNone2PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		//sPowerA
		pDataUser->sPowerA.dForthHavePowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerA.dBackHavePowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerA.dCombNone1PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerA.dCombNone2PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		//sFactorA
		pDataUser->sFactorA.fVolt = d07_str_to_double(&strUsr[i += 8], 4, 3);
		pDataUser->sFactorA.fElec = d07_str_to_double(&strUsr[i += 4], 6, 3);
		pDataUser->sFactorA.fHavePowerRate = d07_str_to_double(&strUsr[i += 6],
				6, 2);
		pDataUser->sFactorA.fNonePowerRate = d07_str_to_double(&strUsr[i += 6],
				6, 2);
		pDataUser->sFactorA.fPowerRateFactor = d07_str_to_double(
				&strUsr[i += 6], 4, 1);
		//sPowerB
		pDataUser->sPowerB.dForthHavePowerIcr = d07_str_to_double(
				&strUsr[i += 4], 8, 6);
		pDataUser->sPowerB.dBackHavePowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerB.dCombNone1PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerB.dCombNone2PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		//sFactorB
		pDataUser->sFactorB.fVolt = d07_str_to_double(&strUsr[i += 8], 4, 3);
		pDataUser->sFactorB.fElec = d07_str_to_double(&strUsr[i += 4], 6, 3);
		pDataUser->sFactorB.fHavePowerRate = d07_str_to_double(&strUsr[i += 6],
				6, 2);
		pDataUser->sFactorB.fNonePowerRate = d07_str_to_double(&strUsr[i += 6],
				6, 2);
		pDataUser->sFactorB.fPowerRateFactor = d07_str_to_double(
				&strUsr[i += 6], 4, 1);
		//sPowerc
		pDataUser->sPowerC.dForthHavePowerIcr = d07_str_to_double(
				&strUsr[i += 4], 8, 6);
		pDataUser->sPowerC.dBackHavePowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerC.dCombNone1PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerC.dCombNone2PowerIcr = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		//sFactorc
		pDataUser->sFactorC.fVolt = d07_str_to_double(&strUsr[i += 8], 4, 3);
		pDataUser->sFactorC.fElec = d07_str_to_double(&strUsr[i += 4], 6, 3);
		pDataUser->sFactorC.fHavePowerRate = d07_str_to_double(&strUsr[i += 6],
				6, 2);
		pDataUser->sFactorC.fNonePowerRate = d07_str_to_double(&strUsr[i += 6],
				6, 2);
		pDataUser->sFactorC.fPowerRateFactor = d07_str_to_double(
				&strUsr[i += 6], 4, 1);
		// sAtime
		pDataUser->sAtime.dAtimeTotal = d07_str_to_double(&strUsr[i += 4], 8,
				6);
		pDataUser->sAtime.dAtimePhaseA = d07_str_to_double(&strUsr[i += 8], 8,
				6);
		pDataUser->sAtime.dAtimePhaseB = d07_str_to_double(&strUsr[i += 8], 8,
				6);
		pDataUser->sAtime.dAtimePhaseC = d07_str_to_double(&strUsr[i += 8], 8,
				6);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_anti_phase(E_D07_TRANS_FLG flag, char *user,
		char *frame)                                    // 逆相序数据格式
{
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_un_balance(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_zone_prog(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(strUsr,
				"%02d%02d%02d%02d%02d%02d"                     //sOccurTime
						...
						"%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d%02d",//sZoneSet2
				//sBefTime
				pDataUser->sOccurTime.YY, pDataUser->sOccurTime.MM,
				pDataUser->sOccurTime.DD, pDataUser->sOccurTime.hh,
				pDataUser->sOccurTime.mm, pDataUser->sOccurTime.ss,
				//operator
				pDataUser->operator[0], pDataUser->operator[1],
				pDataUser->operator[2], pDataUser->operator[3],

				//sZoneSet1
				pDataUser->sZoneSet1[0].MM, pDataUser->sZoneSet1[0].DD,
				pDataUser->sZoneSet1[0].NN, pDataUser->sZoneSet1[1].MM,
				pDataUser->sZoneSet1[1].DD, pDataUser->sZoneSet1[1].NN,
				pDataUser->sZoneSet1[2].MM, pDataUser->sZoneSet1[2].DD,
				pDataUser->sZoneSet1[2].NN, pDataUser->sZoneSet1[3].MM,
				pDataUser->sZoneSet1[3].DD, pDataUser->sZoneSet1[3].NN,
				pDataUser->sZoneSet1[4].MM, pDataUser->sZoneSet1[4].DD,
				pDataUser->sZoneSet1[4].NN, pDataUser->sZoneSet1[5].MM,
				pDataUser->sZoneSet1[5].DD, pDataUser->sZoneSet1[5].NN,
				pDataUser->sZoneSet1[6].MM, pDataUser->sZoneSet1[6].DD,
				pDataUser->sZoneSet1[6].NN, pDataUser->sZoneSet1[7].MM,
				pDataUser->sZoneSet1[7].DD, pDataUser->sZoneSet1[7].NN,
				pDataUser->sZoneSet1[8].MM, pDataUser->sZoneSet1[8].DD,
				pDataUser->sZoneSet1[8].NN, pDataUser->sZoneSet1[9].MM,
				pDataUser->sZoneSet1[9].DD, pDataUser->sZoneSet1[9].NN,
				pDataUser->sZoneSet1[10].MM, pDataUser->sZoneSet1[10].DD,
				pDataUser->sZoneSet1[10].NN, pDataUser->sZoneSet1[11].MM,
				pDataUser->sZoneSet1[11].DD, pDataUser->sZoneSet1[11].NN,
				pDataUser->sZoneSet1[12].MM, pDataUser->sZoneSet1[12].DD,
				pDataUser->sZoneSet1[12].NN, pDataUser->sZoneSet1[13].MM,
				pDataUser->sZoneSet1[13].DD, pDataUser->sZoneSet1[13].NN,
				//sZoneSet2
				pDataUser->sZoneSet2[0].MM, pDataUser->sZoneSet2[0].DD,
				pDataUser->sZoneSet2[0].NN, pDataUser->sZoneSet2[1].MM,
				pDataUser->sZoneSet2[1].DD, pDataUser->sZoneSet2[1].NN,
				pDataUser->sZoneSet2[2].MM, pDataUser->sZoneSet2[2].DD,
				pDataUser->sZoneSet2[2].NN, pDataUser->sZoneSet2[3].MM,
				pDataUser->sZoneSet2[3].DD, pDataUser->sZoneSet2[3].NN,
				pDataUser->sZoneSet2[4].MM, pDataUser->sZoneSet2[4].DD,
				pDataUser->sZoneSet2[4].NN, pDataUser->sZoneSet2[5].MM,
				pDataUser->sZoneSet2[5].DD, pDataUser->sZoneSet2[5].NN,
				pDataUser->sZoneSet2[6].MM, pDataUser->sZoneSet2[6].DD,
				pDataUser->sZoneSet2[6].NN, pDataUser->sZoneSet2[7].MM,
				pDataUser->sZoneSet2[7].DD, pDataUser->sZoneSet2[7].NN,
				pDataUser->sZoneSet2[8].MM, pDataUser->sZoneSet2[8].DD,
				pDataUser->sZoneSet2[8].NN, pDataUser->sZoneSet2[9].MM,
				pDataUser->sZoneSet2[9].DD, pDataUser->sZoneSet2[9].NN,
				pDataUser->sZoneSet2[10].MM, pDataUser->sZoneSet2[10].DD,
				pDataUser->sZoneSet2[10].NN, pDataUser->sZoneSet2[11].MM,
				pDataUser->sZoneSet2[11].DD, pDataUser->sZoneSet2[11].NN,
				pDataUser->sZoneSet2[12].MM, pDataUser->sZoneSet2[12].DD,
				pDataUser->sZoneSet2[12].NN, pDataUser->sZoneSet2[13].MM,
				pDataUser->sZoneSet2[13].DD, pDataUser->sZoneSet2[13].NN);
...
// 有些结构还是不明白
eD07Err trans_d07_data_recd_part_prog(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
#ifdef D07_DEBUG_ON
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_adjust_time(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(strUsr,
				...
				pDataUser->operator[2], pDataUser->operator[3],
				//sBefTime
				pDataUser->sBefTime.YY, pDataUser->sBefTime.MM,
				pDataUser->sBefTime.DD, pDataUser->sBefTime.hh,
				pDataUser->sBefTime.mm, pDataUser->sBefTime.ss,
				//sAftTime
				pDataUser->sAftTime.YY, pDataUser->sAftTime.MM,
				pDataUser->sAftTime.DD, pDataUser->sAftTime.hh,
				pDataUser->sAftTime.mm, pDataUser->sAftTime.ss);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_less_more_loss_elec(E_D07_TRANS_FLG flag,
		char *user, char *frame) {
	INT32 nLenBCD = 115;
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_event_clear(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_demand_clear(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	} else                        // frame to user interface
	{
		...
		pDataUser->operator[3] = d07_str_to_num(&strUsr[i += 2], 2);
		//sTotal
		pDataUser->sTotal.sFortHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sTotal.sFortHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sTotal.sFortHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sFortHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sFortHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sFortHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sBackHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sTotal.sBackHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sTotal.sBackHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sBackHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sBackHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sBackHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad1.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sTotal.sNoneDemandQuad1.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sTotal.sNoneDemandQuad1.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad1.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad1.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad1.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad2.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sTotal.sNoneDemandQuad2.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sTotal.sNoneDemandQuad2.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad2.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad2.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad2.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad3.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sTotal.sNoneDemandQuad3.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sTotal.sNoneDemandQuad3.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad3.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad3.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad3.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad4.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sTotal.sNoneDemandQuad4.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sTotal.sNoneDemandQuad4.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad4.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad4.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sTotal.sNoneDemandQuad4.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		//sPhaseA
		pDataUser->sPhaseA.sFortHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseA.sFortHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseA.sFortHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sFortHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sFortHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sFortHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sBackHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseA.sBackHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseA.sBackHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sBackHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sBackHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sBackHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad1.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseA.sNoneDemandQuad1.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad1.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad1.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad1.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad1.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad2.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseA.sNoneDemandQuad2.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad2.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad2.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad2.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad2.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad3.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseA.sNoneDemandQuad3.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad3.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad3.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad3.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad3.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad4.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseA.sNoneDemandQuad4.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad4.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad4.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad4.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseA.sNoneDemandQuad4.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		//sPhaseB
		pDataUser->sPhaseB.sFortHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseB.sFortHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseB.sFortHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sFortHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sFortHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sFortHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sBackHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseB.sBackHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseB.sBackHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sBackHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sBackHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sBackHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad1.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseB.sNoneDemandQuad1.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad1.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad1.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad1.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad1.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad2.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseB.sNoneDemandQuad2.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad2.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad2.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad2.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad2.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad3.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseB.sNoneDemandQuad3.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad3.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad3.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad3.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad3.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad4.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseB.sNoneDemandQuad4.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad4.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad4.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad4.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseB.sNoneDemandQuad4.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		//sPhaseC
		pDataUser->sPhaseC.sFortHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseC.sFortHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseC.sFortHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sFortHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sFortHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sFortHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sBackHaveDemand.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseC.sBackHaveDemand.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseC.sBackHaveDemand.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sBackHaveDemand.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sBackHaveDemand.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sBackHaveDemand.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad1.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseC.sNoneDemandQuad1.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad1.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad1.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad1.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad1.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad2.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseC.sNoneDemandQuad2.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad2.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad2.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad2.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad2.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad3.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseC.sNoneDemandQuad3.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad3.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad3.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad3.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad3.mm = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad4.fXX_XXXX = d07_str_to_double(
				&strUsr[i += 2], 6, 2);
		pDataUser->sPhaseC.sNoneDemandQuad4.YY = d07_str_to_num(&strUsr[i += 6],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad4.MM = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad4.DD = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad4.hh = d07_str_to_num(&strUsr[i += 2],
				2);
		pDataUser->sPhaseC.sNoneDemandQuad4.mm = d07_str_to_num(&strUsr[i += 2],
				2);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_meter_clear(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	} else                        // frame to user interface
	{
		...
		pDataUser->operator[3] = d07_str_to_num(&strUsr[i += 2], 2);
		//sTotal
		pDataUser->sTotal.dForthHavePower = d07_str_to_double(&strUsr[i += 2],
				8, 6);
		pDataUser->sTotal.dBackHavePower = d07_str_to_double(&strUsr[i += 8], 8,
				6);
		pDataUser->sTotal.dNonePowerQuad1 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sTotal.dNonePowerQuad2 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sTotal.dNonePowerQuad3 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sTotal.dNonePowerQuad4 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		//sPhaseA
		pDataUser->sPhaseA.dForthHavePower = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseA.dBackHavePower = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseA.dNonePowerQuad1 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseA.dNonePowerQuad2 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseA.dNonePowerQuad3 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseA.dNonePowerQuad4 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		//sPhaseB
		pDataUser->sPhaseB.dForthHavePower = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseB.dBackHavePower = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseB.dNonePowerQuad1 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseB.dNonePowerQuad2 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseB.dNonePowerQuad3 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseB.dNonePowerQuad4 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		//sPhaseC
		pDataUser->sPhaseC.dForthHavePower = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseC.dBackHavePower = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseC.dNonePowerQuad1 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseC.dNonePowerQuad2 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseC.dNonePowerQuad3 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
		pDataUser->sPhaseC.dNonePowerQuad4 = d07_str_to_double(&strUsr[i += 8],
				8, 6);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_meter_prog(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(str, "%02d%02d%02d%02d%02d%02d"
				"%02d%02d%02d%02d"
				"%08X%08X%08X%08X%08X"
				"%08X%08X%08X%08X%08X", pDataUser->sProgTime.YY,
				pDataUser->sProgTime.MM, pDataUser->sProgTime.DD,
				pDataUser->sProgTime.hh, pDataUser->sProgTime.mm,
				pDataUser->sProgTime.ss, (UINT32) pDataUser->programer[0],
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_volt_pass_percent(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(strUsr,
				"%06d%06.2f%06.2f%06d%06d%04.1f%02d%02d%02d%02d%04.1f%02d%02d%02d%02d",
				(UINT32) pDataUser->ulMonitorVoltMinutes,
				pDataUser->fVoltPassPercent, pDataUser->fVoltOverLimitPercent,
				(UINT32) pDataUser->ulVoltOverUpperLimitMinutes,
				(UINT32) pDataUser->ulVoltOverLowerLimitMinutes,
				pDataUser->fVoltValueMax, pDataUser->sTimeVoltValueMax.MM,
				pDataUser->sTimeVoltValueMax.DD,
				pDataUser->sTimeVoltValueMax.hh,
				pDataUser->sTimeVoltValueMax.mm, pDataUser->fVoltValueMin,
	...
	} else    // frame to user interface
	{
		...
		pDataUser->fVoltOverLimitPercent = d07_str_to_double(&strUsr[i += 6], 6,
				4);
		pDataUser->ulVoltOverUpperLimitMinutes = d07_str_to_num(&strUsr[i += 6],
				6);
		pDataUser->ulVoltOverLowerLimitMinutes = d07_str_to_num(&strUsr[i += 6],
				6);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_weekday_prog(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_holiday_prog(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(strUsr,
				...
				pDataUser->operator[2], pDataUser->operator[3],
				//sHoliday
				pDataUser->sHoliday[0].YY, pDataUser->sHoliday[0].MM,
				pDataUser->sHoliday[0].DD, pDataUser->sHoliday[0].NN,
				pDataUser->sHoliday[1].YY, pDataUser->sHoliday[1].MM,
				pDataUser->sHoliday[1].DD, pDataUser->sHoliday[1].NN,
				pDataUser->sHoliday[2].YY, pDataUser->sHoliday[2].MM,
				pDataUser->sHoliday[2].DD, pDataUser->sHoliday[2].NN,
				pDataUser->sHoliday[3].YY, pDataUser->sHoliday[3].MM,
				pDataUser->sHoliday[3].DD, pDataUser->sHoliday[3].NN,
				pDataUser->sHoliday[4].YY, pDataUser->sHoliday[4].MM,
				pDataUser->sHoliday[4].DD, pDataUser->sHoliday[4].NN,
				pDataUser->sHoliday[5].YY, pDataUser->sHoliday[5].MM,
				pDataUser->sHoliday[5].DD, pDataUser->sHoliday[5].NN,
				pDataUser->sHoliday[6].YY, pDataUser->sHoliday[6].MM,
				pDataUser->sHoliday[6].DD, pDataUser->sHoliday[6].NN,
				pDataUser->sHoliday[7].YY, pDataUser->sHoliday[7].MM,
				pDataUser->sHoliday[7].DD, pDataUser->sHoliday[7].NN,
				pDataUser->sHoliday[8].YY, pDataUser->sHoliday[8].MM,
				pDataUser->sHoliday[8].DD, pDataUser->sHoliday[8].NN,
				pDataUser->sHoliday[9].YY, pDataUser->sHoliday[9].MM,
				pDataUser->sHoliday[9].DD, pDataUser->sHoliday[9].NN,
				pDataUser->sHoliday[10].YY, pDataUser->sHoliday[10].MM,
				pDataUser->sHoliday[10].DD, pDataUser->sHoliday[10].NN,
				pDataUser->sHoliday[11].YY, pDataUser->sHoliday[11].MM,
				pDataUser->sHoliday[11].DD, pDataUser->sHoliday[11].NN);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_power_prog(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_payoff_prog(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	if (E_D07_TRANS_U2F == flag) {
		...
		sprintf(strUsr,
				...
				pDataUser->operator[2], pDataUser->operator[3],
				//ucWord
				pDataUser->sDay1.DD, pDataUser->sDay1.hh, pDataUser->sDay2.DD,
				pDataUser->sDay2.hh, pDataUser->sDay3.DD, pDataUser->sDay3.hh);
 ...
 *****************************************************************************/
eD07Err trans_d07_data_recd_open_meter_cap(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
	...
	} else                        // frame to user interface
	{
		...
		pDataUser->sEndTime.ss = d07_str_to_num(&strUsr[i += 2], 2);
		//sTotal
		pDataUser->sPowerBeforOpen.dForthHavePower = d07_str_to_double(
				&strUsr[i += 2], 8, 6);
		pDataUser->sPowerBeforOpen.dBackHavePower = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerBeforOpen.dNonePowerQuad1 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerBeforOpen.dNonePowerQuad2 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerBeforOpen.dNonePowerQuad3 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerBeforOpen.dNonePowerQuad4 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		//sPhaseA
		pDataUser->sPowerAfterOpen.dForthHavePower = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerAfterOpen.dBackHavePower = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerAfterOpen.dNonePowerQuad1 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerAfterOpen.dNonePowerQuad2 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerAfterOpen.dNonePowerQuad3 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
		pDataUser->sPowerAfterOpen.dNonePowerQuad4 = d07_str_to_double(
				&strUsr[i += 8], 8, 6);
...
// 同开表盖
eD07Err trans_d07_data_recd_open_btn_box(E_D07_TRANS_FLG flag, char *user,
		char *frame) {
