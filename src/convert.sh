...
#         USAGE: ./conv.sh 
# 
#   DESCRIPTION: 一个支持把整个目录递归转换gb18030为UTF-8的脚本；
# 
#       OPTIONS: ---
...
if [[ $# == 0 ]] ; then
    echo ""
    echo "程序版本：0.1"
    echo "实现功能：批量转换gb18030字符编码至UTF-8；"
...
for n in ${target[@]};do
    # 判断是否是一个文件；
    if [[ -f $n ]] ; then
        iconv -f gb18030 -t UTF-8 $n -o $n 2> /dev/null
    fi
    #判断是否是一个目录
    if [[ -d $n ]] ; then
            find "$n" -name "*.$suffix" | while read line ; do
            iconv -f gb18030 -t UTF-8 "$line" -o "$line"  2> /dev/null
        done
    fi
done
