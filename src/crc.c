...
u16 crc16continue(const u8 *nData, u16 wLength, u16 crc, u8 lastCrc)
{
    u8 nTemp;
    u16 wCRCWord = crc;
...
u16 crcfile(const char* filename, uint8_t *buf, uint32_t bufSize)
{
    uint16_t crc = 0;
    uint32_t offset = 0;
    uint32_t readLen = 0;
    ...
    FRESULT ret = GetFileSize(filename, &fileSize);
    if(ret == FR_OK)
    {
        count = fileSize / bufSize + (((fileSize % bufSize) == 0) ? 0 : 1);
        for(int i = 0, offset = 0; i < count; i++, offset += readLen)
        {
            ret = ReadFileToBuf(filename, offset, buf, bufSize, &readLen);
            if(ret == FR_OK)
            {
                crc = crc16continue(buf, readLen, crc, i);
            }
            else
            {
                break;
...
int main(int argc, char **argv)
{
	...
	crc = crcfile(argv[1], buf, sizeof(buf));
