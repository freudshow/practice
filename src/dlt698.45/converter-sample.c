...
asn_TYPE_descriptor_t PDU_Type;	/* ASN.1 type to be decoded */
...
static inline void
DEBUG_FUNC_LINE(const char* file, const char* func, int line, const char *fmt, ...) {
	va_list ap;
	if(!opt_debug) return;
	fprintf(stderr, "\n[%s][%s()][%d]AD: ", file, func, line);
	va_start(ap, fmt);
...
#define	DEBUG_FUNC_LINE(format, ...)	DEBUG_FUNC_LINE(FILE_LINE, format, ##__VA_ARGS__)
	...
	for(num = 0; num < number_of_iterations; num++) {
	  ...
	  for(ac_i = 0; ac_i < ac; ac_i++) {
		...
		case OUT_DER:
			...
			DEBUG_FUNC_LINE("Encoded in %ld bytes of DER", (long)erv.encoded);
			break;
		case OUT_PER:
			...
			DEBUG_FUNC_LINE("Encoded in %ld bits of UPER", (long)erv.encoded);
			break;
		}
...
buffer_dump() {
	...
	if(!opt_debug) return;
	DEBUG_FUNC_LINE("Buffer: { d=%p, o=%ld, l=%ld, u=%ld, a=%ld, s=%ld }",
		DynamicBuffer.data,
...
buffer_shift_left(size_t offset, int bits) {
	...
	DEBUG_FUNC_LINE("Shifting left %d bits off %ld (o=%ld, u=%ld, l=%ld)",
		bits, (long)offset,
		(long)DynamicBuffer.offset,
		(long)DynamicBuffer.unbits,
		(long)DynamicBuffer.length);
	...
	if(offset) {
		...
		DEBUG_FUNC_LINE("oleft: %c%c%c%c%c%c%c%c",
			((ptr[-1] >> 7) & 1) ? '1' : '0',
			((ptr[-1] >> 6) & 1) ? '1' : '0',
			((ptr[-1] >> 5) & 1) ? '1' : '0',
			((ptr[-1] >> 4) & 1) ? '1' : '0',
		...
		DEBUG_FUNC_LINE("oriht: %c%c%c%c%c%c%c%c",
			((ptr[0] >> 7) & 1) ? '1' : '0',
			((ptr[0] >> 6) & 1) ? '1' : '0',
			((ptr[0] >> 5) & 1) ? '1' : '0',
			((ptr[0] >> 4) & 1) ? '1' : '0',
		...
		DEBUG_FUNC_LINE("mriht: %c%c%c%c%c%c%c%c",
			((right >> 7) & 1) ? '1' : '0',
			((right >> 6) & 1) ? '1' : '0',
			((right >> 5) & 1) ? '1' : '0',
			((right >> 4) & 1) ? '1' : '0',
		...
		DEBUG_FUNC_LINE("after: %c%c%c%c%c%c%c%c",
			((ptr[-1] >> 7) & 1) ? '1' : '0',
			((ptr[-1] >> 6) & 1) ? '1' : '0',
			((ptr[-1] >> 5) & 1) ? '1' : '0',
			((ptr[-1] >> 4) & 1) ? '1' : '0',
	...
	*ptr <<= bits;
	...
	DEBUG_FUNC_LINE("Unbits [%d=>", (int)DynamicBuffer.unbits);
	...
	DEBUG_FUNC_LINE("Shifted. Now (o=%ld, u=%ld l=%ld)",
		(long)DynamicBuffer.offset,
		(long)DynamicBuffer.unbits,
		(long)DynamicBuffer.length);
...
static void add_bytes_to_buffer(const void *data2add, size_t bytes) {
	...
	DEBUG_FUNC_LINE("=> add_bytes(%ld) { o=%ld l=%ld u=%ld, s=%ld }",
		(long)bytes,
		(long)DynamicBuffer.offset,
		(long)DynamicBuffer.length,
		(long)DynamicBuffer.unbits,
	...
	>= (DynamicBuffer.offset + DynamicBuffer.length + bytes)) {
		DEBUG_FUNC_LINE("\tNo buffer reallocation is necessary");
	} else if(bytes <= DynamicBuffer.offset) {
		DEBUG_FUNC_LINE("\tContents shifted by %ld", DynamicBuffer.offset);
		...
		DEBUG_FUNC_LINE("\tBuffer reallocated to %ld (%d time)",
...
data_decode_from_file(asn_TYPE_descriptor_t *pduType, FILE *file, const char *name, ssize_t suggested_bufsize, int on_first_pdu) {
	...
	DEBUG_FUNC_LINE("Processing %s", name);
	...
	for(tolerate_eof = 1;	/* Allow EOF first time buffer is non-empty */
		...
		DEBUG_FUNC_LINE("Decoding %ld bytes", (long)i_size);
		...
		DEBUG_FUNC_LINE("decode(%ld) consumed %ld+%db (%ld), code %d",
			(long)DynamicBuffer.length,
		...
		case RC_OK:
			...
			DEBUG_FUNC_LINE("RC_OK, finishing up with %ld+%d",
				(long)rval.consumed, ecbits);
			return structure;
		case RC_WMORE:
			DEBUG_FUNC_LINE("RC_WMORE, continuing read=%ld, cons=%ld "
				" with %ld..%ld-%ld..%ld",
				(long)rd,
				(long)rval.consumed,
				(long)DynamicBuffer.offset,
	...
	DEBUG_FUNC_LINE("Clean up partially decoded structure");
	ASN_STRUCT_FREE(*pduType, structure);
	...
	|| new_offset - old_offset > ((iform == INP_XER)?sizeof("\r\n")-1:0)
		...
		DEBUG_FUNC_LINE("ofp %d, no=%ld, oo=%ld, dbl=%ld",
...
#define	BPROB(b)	((((double)random() / RAND_MAX) < prob) ? b : 0)
		...
		if(byte != *ptr) {
			DEBUG_FUNC_LINE("Junk buf[%d] %02x -> %02x",
				ptr - buf, *ptr, byte);
			*ptr = byte;
		}
	}
