...
void MD5_Update(MD5Context * context, unsigned char * buf, int len) {
	...
	if ((context->count[0] += ((unsigned int) len << 3))
			< ((unsigned int) len << 3))
		context->count[1]++;
...
void MD5_File(char * filename, char * md5str) {
	FILE *file;
	MD5Context context;
	...
	if (!(file = fopen(filename, "rb")))
		fprintf(stderr, "%s can't be opened %d %s\n", filename, errno,
				strerror(errno));
	else {
		MD5_Init(&context);
...
//           printf("%x",buff[i] & 0x0F);
			sprintf(&md5str[j++], "%X", (buff[i] & 0xF0) >> 4);
			sprintf(&md5str[j++], "%X", buff[i] & 0x0F);
