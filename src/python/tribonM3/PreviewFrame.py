...
#-------------------------------------------------------------------------------
	def GetDwgNameStmt(self):
		dwg_type   = self.GetDwgType()
		text_keyin = self.textCtrlKeyIn.GetValue().upper()
		if text_keyin == "*" or text_keyin == "":
			stmt_get_dwgname = "DRA('@"+dwg_type+"'*).NAM"
			return stmt_get_dwgname
		if text_keyin[0] == '*':
			text_keyin = text_keyin[1:len(text_keyin)]
		split = text_keyin.split('*')
		str = split[0]+'@'+dwg_type
		stmt_get_dwgname = "DRA('"+str+"'*).NAM"
		return stmt_get_dwgname

#-------------------------------------------------------------------------------
	def GetPieceListStmt(self):
		dwg_type = self.GetDwgType()
		text_keyin = self.textCtrlKeyIn.GetValue().upper()
		if text_keyin == "*" or text_keyin == "":
			stmt_get_dwgname = "DRA('@"+dwg_type+"'*).NAM"
		...
		lenth = len(text_keyin)
		if text_keyin[0] == '*':
			text_keyin = text_keyin[1:lenth]
		lenth = len(text_keyin)
		if text_keyin[lenth-1] == '*':
			text_keyin = text_keyin[0:lenth-1]
		str=""
		for i in text_keyin:
			if i == '*':
				str += '\''+i+'\''
				...
				str += i
		stmt_get_dwgname = "DRA(*'"+str+'@'+dwg_type+"'*).NAM" 
		return stmt_get_dwgname
...
#-------------------------------------------------------------------------------
	def OnButtonSearchByPieceButton(self, event):
		self.listCtrlDwgList.DeleteAllItems()
		stmt_piece_list 	= self.GetPieceListStmt()
...
#-------------------------------------------------------------------------------
	def OnButtonChangePageNoButton(self, event):
		mode = kcs_draft.kcsOPENMODE_READWRITE
		self.OpenCurrentDwg(mode)
